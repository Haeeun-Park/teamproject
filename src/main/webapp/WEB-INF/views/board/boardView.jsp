...
<div id="main_container">
	...
	<div class="board">
	  	...
	  	<div class="text">
	  		<c:choose>
	  		  <c:when test="${empty board.savefilename}">
	  		  	<img src="upload/noname.jpg" width="250"/><br/>
	  		  	이미지가 없습니다
	  		  </c:when>
	  		  <c:otherwise>
	  		    <img src="upload/${board.savefilename}" width="350"/><br/>
	  		    ${board.image}
	  		  </c:otherwise>
	  		</c:choose>
	  	</div>
	  ...
	  <div class="login-button">
	  	...
	  	onClick="deleteBoardConfirm('${board.num}')"/>
	  	<input type="button" class="btn-login" value="비밀번호수정"
	  	onClick="updatePass('${board.num}')"/>
	  	<input type="button" class="btn-login" value="목록으로"
	...
	<form action="addReply" method="post" name="addRep">
	 ...
	 <c:choose>
	   <c:when test="${replyList.size()==0}">
	   	<div class="reply_row">
	   		<div class="reply_col">
	   			아직 댓글이 하나도 없습니다. 댓글을 남겨서 작성자를 응원
	   		</div>
	   	</div>
	   	</c:when>
